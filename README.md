...
| `tests/` | 25 parser tests and 694 UI assertions. Not optional — run them before you push. |
...
node tests/ui_smoke.js           # 694 assertions
...
- **Map** is a schematic of the con hotels, transit-map style and not to scale: Peachtree St up the left, the Hyatt, Marriott and Hilton in a skybridged row east of it, the Mart and the Westin west of it with their own skybridge, the Courtland below the Hilton, and Hardy Ivy Park above the Hyatt. Hotel level only; streams and offsite venues are not on it. A row of day chips above it picks the con day, starting on today with the same 5 AM boundary as the rest of the app. Each hotel wears a gold pill with the number of your picks there that day. Tap a hotel (or its pill) for those picks as ordinary rows, star and detail sheet included; a hotel with none offers a button that searches it on that day. When the chosen day is today, a solid gold ring marks the hotel of the pick that is on now, a pulsing one the hotel of the next pick, and, while a pick is on, a dashed gold line runs beneath the blocks from the hotel you are in to the next one. A caption under the map says where and when, with the same leave-by the Now tab shows, or what is next when the next pick is streaming or offsite; picks at venues the map does not draw are counted under it. A thin gold line threads the day's picks through their hotels in time order, so the shape of the day is visible at a glance.
