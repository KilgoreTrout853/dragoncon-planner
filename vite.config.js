...
import { dcBuild, dcYear } from "./build/vite-dc.js";
...
/* One inlined dist/index.html beside the files in public/ and a copy of the
   ...
   #39, #49). The same build is deployed at two subpaths, so every URL in it
   is relative. */
export default defineConfig({
  ...
  /* Order matters: the year first, which names the data the page imports;
     then the inlining; then this project's fix-ups. */
  plugins: [dcYear(), viteSingleFile(), dcBuild()],
});
