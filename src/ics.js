...
function downloadICS(mine, filename) {
  ...
  const lines = ["BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//dragoncon-planner//EN", "CALSCALE:GREGORIAN", `X-WR-CALNAME:Dragon Con ${CON.year}`,
  ...
  mine.forEach(ev => {
    ...
    lines.push("BEGIN:VEVENT", `UID:dc${YY}-${ev.id}@dragoncon-planner`, `DTSTAMP:${stamp}`,
