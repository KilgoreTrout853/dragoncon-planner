...
| `docs/pipeline/` | Evidence for Pipeline shape (ROADMAP tentpole 1): `history-2026.md`, how the 2026 schedule changed from commit to commit on `main` and how scrape.yml ran, written by `tools/schedule_history.py`. A record, not held fresh by CI: a shallow checkout has no history, and the history will not change. |
