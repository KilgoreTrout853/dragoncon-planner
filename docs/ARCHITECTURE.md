...
- The client script is split: twenty-five modules in one order, and the
  ...
  which is why `loading.js`, not the shell, holds the two functions that
  measure the header: `updateFresh()` calls one of them.
