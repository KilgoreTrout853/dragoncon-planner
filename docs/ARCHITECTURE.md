...
| `tests/test_room_census.py` | The room census's exact match, each combined-string rule and each other shape on the strings that showed them, the chains they make, and a render of an inline schedule and registry under two hash seeds that leaves both inputs as they were. It reads neither `data/` nor `docs/venues/`. |
...
| `docs/pipeline/` | Pipeline shape (ROADMAP tentpole 1): `contract.md`, the design note for DECISIONS #41-#48 - the 2027 pipeline's files, with their writers and readers, the raw row, the ledger, the change log and the stages - decided, not yet built; and its evidence, `history-2026.md`, how the 2026 schedule changed from commit to commit on `main` and how scrape.yml ran, written by `tools/schedule_history.py`. The history is a record, not held fresh by CI: a shallow checkout has no history, and the history will not change. |
| `docs/venues/` | The venues registry (DECISIONS #21, #27, #28): `registry.json`, hand-edited - every hotel × level where programming happens, its rooms as the schedule names them, which published floor plan covers the level, and the state of our own drawing; `README.md`, that checklist rendered, with the notes that do not fit a cell; `drawings/`, our schematics, one draft so far; and `census-2026.md`, the room census - every room string of the frozen schedule against the registry, written by `tools/room_census.py`, the one reader of the registry so far. The census is a record, not held fresh by CI: a registry edit leaves it stale until the script runs again. |
